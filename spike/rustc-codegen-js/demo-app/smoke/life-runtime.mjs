...
// that has to be kept in step with the view by somebody noticing, and the first
// thing a mistake in it would look like is a bug in the island.
//
// So the graph is parsed from the template string the chunk itself declares. The
// parser handles what a `view!` template contains and nothing else -- elements
// with double quoted attributes, void elements, text and the `<!$>` markers -- and
// throws on anything it does not know rather than guessing, because a silently
// mis-parsed node is a walk that lands somewhere plausible and wrong.
//
// What is deliberately NOT modelled: layout, event dispatch and the hydration
// registry. A handler is a property the emitter assigns, so a check calls it; the
// registry is what the browser checklist in README.md covers.
...
///
/// The emitter reaches `<span class="life-generation">` as the SECOND child of the
/// readout, because `"generation "` is the first. Dropping text from the graph
/// would move every hole in this island one node to the left.
function text(data) {
	return { tag: "#text", data, childNodes: [], firstChild: null, nextSibling: null };
}

/// The `<!$>` and `<!/>` a hole is bracketed by, and the `<!>` that keeps two
/// text nodes apart. Nodes for the same reason text is.
...
export function parse(html) {
	...
	while (at < html.length) {
		...
		if (!/^[a-z][a-z0-9-]*$/.test(tag)) {
			throw new Error(`the life stub does not parse the tag ${JSON.stringify(tag)}`);
		}
...
///
/// The island's root is the server's node; every row the grid's loop builds is
/// created, because the server's copy of it is the one being replaced.
let claimed = false;
...
export function setAttribute(node, name, value) {
	...
	// A `<button value="33">` reports "33" as `event.target.value`, which is how
	// the grid's one handler learns which cell was clicked. Reflecting the
	// attribute into the property is that, and it is the only property in this
	// island that a handler reads.
